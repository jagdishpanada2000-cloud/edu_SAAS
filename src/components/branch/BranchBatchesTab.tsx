...
import { Download, Upload, UserPlus, CheckCircle2, Users, Trash2, BookOpen, Plus, ArrowLeft, Pencil } from 'lucide-react'
...
export function BranchBatchesTab({ branchId }: Props) {
  const { batches, addBatch, updateBatch, deleteBatch } = useBatches(branchId)
  const { courses } = useCourses()

  const [view, setView] = React.useState<'list' | 'detail'>('list')
  const [selectedBatch, setSelectedBatch] = React.useState<Batch | null>(null)

  const { students: batchStudents, addBatchStudent, bulkAddBatchStudents, removeBatchStudent, updateBatchStudent } =
    useBatchStudents(view === 'detail' ? selectedBatch?.id : undefined)
  ...
  const [isEditBatchOpen, setIsEditBatchOpen] = React.useState(false)
  const [editBatchSubmitted, setEditBatchSubmitted] = React.useState(false)
  const [editingStudent, setEditingStudent] = React.useState<{ id: string; full_name?: string; phone?: string; parent_phone?: string } | null>(null)
  const [editStudentSubmitted, setEditStudentSubmitted] = React.useState(false)
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const rows = XLSX.utils.sheet_to_json<{ email?: string; full_name?: string; phone?: string; parent_phone?: string }>(wb.Sheets[wb.SheetNames[0]])
      const entries = rows
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              <button onClick={() => setView('list')} className="p-2 rounded-lg text-slate-400 hover:text-[#22c55e] hover:bg-[#22c55e]/10 transition-all">
                <ArrowLeft size={20} />
              </button>
              <div>
                <h2 className="text-xl font-bold">{selectedBatch.name}</h2>
                <p className="text-xs text-slate-500">
                  {selectedBatch.courses?.name ?? '—'}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button onClick={() => { setEditBatchSubmitted(false); setIsEditBatchOpen(true) }}
                className="border border-[#2d2d2d] hover:border-[#22c55e]/50 text-slate-400 hover:text-[#22c55e] text-xs font-bold px-3 py-2 rounded-lg transition-all flex items-center gap-2">
              ...
              <label className="border border-[#2d2d2d] hover:border-[#22c55e]/50 text-slate-400 hover:text-[#22c55e] text-xs font-bold px-3 py-2 rounded-lg transition-all flex items-center gap-2 cursor-pointer">
                <Upload size={14} /> Import Excel
                <input ref={importRef} type="file" accept=".xlsx,.xls,.csv" className="hidden" onChange={handleImport} />
              </label>
              <button onClick={() => { setStudentSubmitted(false); setIsAddStudentOpen(true) }}
                className="bg-[#22c55e] hover:bg-[#22c55e]/90 text-black text-xs font-bold px-4 py-2 rounded-lg transition-all flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[
              { label: 'Course', val: selectedBatch.courses?.name ?? '—' },
              { label: 'Duration', val: selectedBatch.duration ?? '—' },
              { label: 'Students', val: batchStudents.length.toString() },
            ].map((item, i) => (
              <div key={i} className="bg-[#1b1b1b] border border-[#2d2d2d] rounded-xl p-4">
                      ...
                      <tr key={s.id} className="hover:bg-white/5 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-[#22c55e]/10 flex items-center justify-center">
                              <span className="text-[#22c55e] font-bold text-sm">{(s.full_name ?? s.email)[0].toUpperCase()}</span>
                            </div>
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => { setEditStudentSubmitted(false); setEditingStudent({ id: s.id, full_name: s.full_name, phone: s.phone, parent_phone: s.parent_phone }) }}
                              className="text-slate-400 hover:text-[#22c55e] p-2 transition-colors" title="Edit student">
                              <Pencil size={15} />
                            </button>
                            <button onClick={() => { if (confirm(`Remove ${s.email} from this batch?`)) removeBatchStudent(s.id).catch(() => alert('Failed to remove student')) }}
                              className="text-slate-400 hover:text-red-500 p-2 transition-colors" title="Remove from batch">
                              <Trash2 size={16} />
                            </button>
                          </div>
                        </td>
        ...
        <Modal isOpen={isEditBatchOpen} onClose={() => setIsEditBatchOpen(false)} title="Edit Batch">
          {editBatchSubmitted ? (
            ...
            <form onSubmit={handleEditBatch} className="space-y-4">
              <div><label className={lbl}>Batch Name <span className="text-red-400">*</span></label>
                <input name="name" required type="text" className={inp} defaultValue={selectedBatch?.name} /></div>
              ...
              <div><label className={lbl}>Description (optional)</label>
                <textarea name="description" rows={2} className={inp} defaultValue={selectedBatch?.description ?? ''} /></div>
              <div><label className={lbl}>Duration</label>
                <input name="duration" type="text" className={inp} defaultValue={selectedBatch?.duration ?? ''} placeholder="e.g. 2026-2027" /></div>
              <button type="submit" className="w-full bg-[#22c55e] text-black font-bold py-3 rounded-lg hover:opacity-90 transition-opacity mt-4">Save Changes</button>
            </form>
          )}
        </Modal>
        ...
        <Modal isOpen={isAddStudentOpen} onClose={() => setIsAddStudentOpen(false)} title="Add Student to Batch">
          {studentSubmitted ? (
            ...
            <form onSubmit={handleAddStudent} className="space-y-4">
              <div><label className={lbl}>Student Gmail <span className="text-red-400">*</span></label>
              ...
              <div><label className={lbl}>Full Name</label>
                <input name="full_name" type="text" className={inp} placeholder="e.g. Riya Patel" /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className={lbl}>Student Phone</label>
                  <input name="phone" type="tel" className={inp} placeholder="+91 98765 43210" /></div>
                <div><label className={lbl}>Parent Phone</label>
                  <input name="parent_phone" type="tel" className={inp} placeholder="+91 98765 43211" /></div>
              </div>
              <button type="submit" className="w-full bg-[#22c55e] text-black font-bold py-3 rounded-lg hover:opacity-90 transition-opacity mt-4">Add to Batch</button>
            </form>
          )}
        </Modal>
        ...
        <Modal isOpen={!!editingStudent} onClose={() => setEditingStudent(null)} title="Edit Student">
          {editStudentSubmitted ? (
            ...
            <form onSubmit={handleEditStudent} className="space-y-4">
              <div><label className={lbl}>Full Name</label>
                <input name="full_name" type="text" className={inp} defaultValue={editingStudent?.full_name ?? ''} placeholder="e.g. Riya Patel" /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className={lbl}>Student Phone</label>
                  <input name="phone" type="tel" className={inp} defaultValue={editingStudent?.phone ?? ''} /></div>
                <div><label className={lbl}>Parent Phone</label>
                  <input name="parent_phone" type="tel" className={inp} defaultValue={editingStudent?.parent_phone ?? ''} /></div>
              </div>
              <button type="submit" className="w-full bg-[#22c55e] text-black font-bold py-3 rounded-lg hover:opacity-90 transition-opacity mt-4">Save Changes</button>
            </form>
          )}
      ...
      <section className="bg-[#1b1b1b] rounded-xl border border-[#2d2d2d] overflow-hidden">
        <div className="p-6 border-b border-[#2d2d2d] flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-slate-400">Manage class batches for your branch</p>
          </div>
          <button onClick={() => { setCreateSubmitted(false); setIsCreateOpen(true) }}
              ...
              <tbody className="divide-y divide-[#2d2d2d]">
                {batches.map(batch => (
                  <tr key={batch.id} className="hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center font-bold bg-[#22c55e]/10 text-[#22c55e]">{batch.name[0]}</div>
                        <p className="font-medium">{batch.name}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-400">{batch.courses?.name ?? '—'}</td>
                    <td className="px-6 py-4 text-sm text-slate-400">{batch.duration ?? '—'}</td>
                    <td className="px-6 py-4 text-sm text-slate-500 max-w-[180px] truncate">{batch.description ?? '—'}</td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => { setSelectedBatch(batch); setView('detail') }}
                          className="text-xs font-bold px-3 py-1.5 bg-[#22c55e]/10 text-[#22c55e] rounded-lg hover:bg-[#22c55e]/20 transition-colors flex items-center gap-1">
                          <Users size={13} /> Manage Students
                        </button>
                        <button onClick={() => { if (confirm(`Delete "${batch.name}"?`)) deleteBatch(batch.id).catch(() => alert('Failed to delete batch')) }}
      ...
      <Modal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Create New Batch">
        {createSubmitted ? (
          ...
          <form onSubmit={handleCreateBatch} className="space-y-4">
            <div><label className={lbl}>Batch Name <span className="text-red-400">*</span></label>
              <input name="name" required type="text" className={inp} placeholder="e.g. Morning Batch 2025" /></div>
            <div><label className={lbl}>Course (optional)</label>
            ...
            <div><label className={lbl}>Description (optional)</label>
              <textarea name="description" rows={2} className={inp} placeholder="Brief batch description..." /></div>
            <div><label className={lbl}>Duration (e.g. 2026-2027)</label>
              <input name="duration" type="text" className={inp} placeholder="e.g. 2026-2027" /></div>
            <button type="submit" className="w-full bg-[#22c55e] text-black font-bold py-3 rounded-lg hover:bg-[#16a34a] transition-colors mt-4">Create Batch</button>
          </form>
        )}
      </Modal>
    </>
